...
class Network {
  ...
  setupNetwork() {
    ...
    this.inputTensor = tf.layers.dense({
      units: 3,
      inputDim: 3,
  ...
  train() {
    return this.model
      .fit(this.trainingData.input, this.trainingData.target, {
      ...
      .then(history => {
        ...
        const avgLoss =
          losses.reduce((acc, val) => acc + val, 0) / losses.length;

        const accuracies = history.history.acc;
        const avgAccuracy =
          accuracies.reduce((acc, val) => acc + val, 0) / accuracies.length;
  ...
  predict(input) {
    ...
    return output.data().then(data => {
      return data.map(v => Math.max(Math.min(v, 1), 0));
    });
  }
